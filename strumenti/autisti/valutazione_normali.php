...
$stmt = $db->prepare("
    SELECT r.Cognome, r.Nome, r.Mail, r.Codice, a.DataInizio 
    FROM rubrica r
    LEFT JOIN AUTISTI_RIENTRI a ON r.IDUtente = a.IDUtente
    WHERE r.IDUtente = ?
");
$stmt->bind_param("i", $candidato_id);
$stmt->execute();
$stmt->bind_result($candidato_cognome, $candidato_nome, $candidato_mail, $candidato_codice, $candidato_datarientri);
$stmt->fetch();
$stmt->close();
...
<head>
    ...
    <title>Valutazione NORMALI</title>
...
<body>
<div class="container">
    <h2>SCHEDA DI VALUTAZIONE NORMALI</h2>
    <br>
    <form method="post" action="genera_pdf_prova_normali.php">
        ...
        <hr>
        <div class="form-group">
            <label>Dimostra attenzione nelle manovre?</label>
            <div class="rating">
                <?php for ($i = 1; $i <= 5; $i++): ?>
        ...
        <div class="form-group">
            <label>Fluidità di guida</label>
            <div class="rating">
                <?php for ($i = 1; $i <= 5; $i++): ?>
                    <input type="radio" name="fluidita" id="fluidita<?= $i ?>" value="<?= $i ?>" required>
                    <label for="fluidita<?= $i ?>"><?= $i ?></label>
                <?php endfor; ?>
            </div>
        </div>

        <div class="form-group">
            <label>Come effettua il parcheggio?</label>
            <div class="rating">
                <?php for ($i = 1; $i <= 5; $i++): ?>
                    <input type="radio" name="precisione" id="precisione<?= $i ?>" value="<?= $i ?>" required>
                    <label for="precisione<?= $i ?>"><?= $i ?></label>
                <?php endfor; ?>
            </div>
        </div>

        <div class="form-group">
            <label>Conoscenza minima della toponomastica</label>
            <div class="rating">
                <?php for ($i = 1; $i <= 5; $i++): ?>
